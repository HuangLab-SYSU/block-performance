...
import { key_save as target_fun } from "./export.js";
import { rpc_key_save as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { username: string; address_list: string[]; tag_list: string[]; private_key: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { username: string; address_list: string[]; tag_list: string[]; private_key: string } = {
