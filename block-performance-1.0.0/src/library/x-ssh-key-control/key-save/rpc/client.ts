...
export async function rpc_key_save<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_key_save");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-ssh-key-control/key-save`,
                data: input
            },
            {
