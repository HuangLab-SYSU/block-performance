...
import { key_disabled_set as target_fun } from "./export.js";
import { rpc_key_disabled_set as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { id: string; disabled: boolean }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { id: string; disabled: boolean } = {
