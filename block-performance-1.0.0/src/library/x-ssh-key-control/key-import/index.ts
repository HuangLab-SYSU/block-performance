...
import { key_import as target_fun } from "./export.js";
import { rpc_key_import as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { private_key_filename: string; username: string; address_list: string[]; tag_list: string[] }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { private_key_filename: string; username: string; address_list: string[]; tag_list: string[] } = {
            private_key_filename: await input_private_key_filename(log.sub("private_key_filename"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_private_key_filename<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_private_key_filename");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("private_key_filename", { allow_empty: true });
        return cb.ok(v);
    }
