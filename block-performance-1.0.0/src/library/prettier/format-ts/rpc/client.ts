...
export async function rpc_format_ts<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_format_ts");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/prettier/format-ts`,
                data: input
            },
            {
