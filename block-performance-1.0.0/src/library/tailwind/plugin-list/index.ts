...
import { plugin_list as target_fun } from "./export.js";
import { rpc_plugin_list as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { css_file: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { css_file: string } = {
            css_file: await input_css_file(log.sub("css_file"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_css_file<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_css_file");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("css_file", { allow_empty: true });
        return cb.ok(v);
    }
}
