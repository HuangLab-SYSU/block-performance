...
import { npm_package_analyze as target_fun } from "./export.js";
import { rpc_npm_package_analyze as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { input_dir: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { input_dir: string } = {
            input_dir: await input_input_dir(log.sub("input_dir"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_input_dir<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_input_dir");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("input_dir", { allow_empty: true });
        return cb.ok(v);
    }
}
