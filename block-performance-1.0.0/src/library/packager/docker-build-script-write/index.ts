...
import { docker_build_script_write as target_fun } from "./export.js";
import { rpc_docker_build_script_write as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { name: string; version: string; output_dir: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { name: string; version: string; output_dir: string } = {
