...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            output_dir: await input_output_dir(log.sub("output_dir"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_npm_script<R>(plog: Logger, cb: { ok: (v: { dev: string; start: string; stop: string }) => R; fail: (err: Error) => R }): Promise<R> {
    ...
    async function input_output_dir<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_output_dir");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("output_dir", { allow_empty: true });
        return cb.ok(v);
    }
}
