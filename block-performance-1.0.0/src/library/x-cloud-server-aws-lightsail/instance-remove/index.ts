...
import { instance_remove as target_fun } from "./export.js";
import { rpc_instance_remove as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { id: string; credentials: { accessKeyId: string; secretAccessKey: string } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { id: string; credentials: { accessKeyId: string; secretAccessKey: string } } = {
