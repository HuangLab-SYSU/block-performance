...
import { code_file_write as target_fun } from "./export.js";
import { rpc_code_file_write as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { filename: string; text: string; format: boolean }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { filename: string; text: string; format: boolean } = {
            ...
            format: await input_format(log.sub("format"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_format<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_format");
        const v = await prompts.input_boolean("format");
        return cb.ok(v);
    }
}
