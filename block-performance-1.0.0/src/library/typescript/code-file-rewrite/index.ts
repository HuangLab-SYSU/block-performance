...
import { code_file_rewrite as target_fun } from "./export.js";
import { rpc_code_file_rewrite as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { filename: string; old_snippet: string; new_snippet: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { filename: string; old_snippet: string; new_snippet: string } = {
            filename: await input_filename(log.sub("filename"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            old_snippet: await input_old_snippet(log.sub("old_snippet"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            new_snippet: await input_new_snippet(log.sub("new_snippet"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_old_snippet<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_old_snippet");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("old_snippet", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_new_snippet<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_new_snippet");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("new_snippet", { allow_empty: true });
        return cb.ok(v);
    }
}
