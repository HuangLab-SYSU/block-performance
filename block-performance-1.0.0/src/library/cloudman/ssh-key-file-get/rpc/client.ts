...
export async function rpc_ssh_key_file_get<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_ssh_key_file_get");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cloudman/ssh-key-file-get`,
                data: input
            },
            {
                ok: (rpc_result: RpcResult) => {
                    ...
                    if (rpc_result.without_key_file) {
