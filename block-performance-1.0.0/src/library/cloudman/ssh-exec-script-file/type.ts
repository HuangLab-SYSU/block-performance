...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "local_script_file") return;
        ...
        log.println("v.local_script_file must be string");
        if (typeof v.local_script_file !== "string") {
            throw new Error("v.local_script_file is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
