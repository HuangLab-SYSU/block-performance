...
import { ssh_exec_script_file as target_fun } from "./export.js";
import { rpc_ssh_exec_script_file as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; local_script_file: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; local_script_file: string } = {
            ...
            local_script_file: await input_local_script_file(log.sub("local_script_file"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_local_script_file<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_local_script_file");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("local_script_file", { allow_empty: true });
        return cb.ok(v);
    }
}
