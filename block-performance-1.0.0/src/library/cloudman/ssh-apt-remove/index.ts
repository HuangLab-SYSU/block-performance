...
import { ssh_apt_remove as target_fun } from "./export.js";
import { rpc_ssh_apt_remove as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; app_list: string[] }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; app_list: string[] } = {
