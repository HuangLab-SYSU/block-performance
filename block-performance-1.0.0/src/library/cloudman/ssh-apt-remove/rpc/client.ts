...
export async function rpc_ssh_apt_remove<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_ssh_apt_remove");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cloudman/ssh-apt-remove`,
                data: input
            },
            {
