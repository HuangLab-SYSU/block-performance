...
import { scp_update_text as target_fun } from "./export.js";
import { rpc_scp_update_text as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: { remote_ssh_auth: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string } }; remote_path: string } =
            {
                ...
                remote_path: await input_remote_path(log.sub("remote_path"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_remote_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_remote_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("remote_path", { allow_empty: true });
        return cb.ok(v);
    }
}
