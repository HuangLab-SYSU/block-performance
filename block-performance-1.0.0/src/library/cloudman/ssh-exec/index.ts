...
import { ssh_exec as target_fun } from "./export.js";
import { rpc_ssh_exec as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; command: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; command: string } = {
            ...
            command: await input_command(log.sub("command"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_command<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_command");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("command", { allow_empty: true });
        return cb.ok(v);
    }
}
