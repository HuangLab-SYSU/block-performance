...
import { ssh_exec_script as target_fun } from "./export.js";
import { rpc_ssh_exec_script as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; script: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { username: string; address: string; ssh_auth_key: { os?: boolean; store?: boolean; file?: string }; script: string } = {
            ...
            script: await input_script(log.sub("script"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_script<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_script");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("script", { allow_empty: true });
        return cb.ok(v);
    }
}
