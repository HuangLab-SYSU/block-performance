...
import { rsync_pull as target_fun } from "./export.js";
import { rpc_rsync_pull as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            from_remote_dir: await input_from_remote_dir(log.sub("from_remote_dir"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            to_local_dir: await input_to_local_dir(log.sub("to_local_dir"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_from_remote_dir<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_from_remote_dir");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("from_remote_dir", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_to_local_dir<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_to_local_dir");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("to_local_dir", { allow_empty: true });
        return cb.ok(v);
    }
}
