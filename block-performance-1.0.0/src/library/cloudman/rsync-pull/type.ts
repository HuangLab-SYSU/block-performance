...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "from_remote_dir") return;
            if (field === "to_local_dir") return;
        ...
        log.println("v.from_remote_dir must be string");
        if (typeof v.from_remote_dir !== "string") {
            throw new Error("v.from_remote_dir is not string");
        }

        log.println("v.to_local_dir must be string");
        if (typeof v.to_local_dir !== "string") {
            throw new Error("v.to_local_dir is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
