...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    ...
    // - input.from_remote_dir: "/a/b/"
    // - input.to_local_dir: "/a/b" (no trailing slash)
    //
    // [WHY?]
    ...
    //
    // this is right:
    // - input.to_local_dir: /some-local-path/app-name-version
    // - input.from_remote_dir: /some-remote-path/app-name-version
    //
    // this is wrong:
    // - input.to_local_dir: /some-local-path/app-name-version
    // - input.from_remote_dir: /some-remote-path/

    const to_local_dir = path.resolve(input.to_local_dir);
    const from_remote_dir = path.resolve(input.from_remote_dir) + "/";
    log.variable("to_local_dir", to_local_dir);
    log.variable("from_remote_dir", from_remote_dir);
    ...
    await run(
        log,
        {
            ...
            args: [
                ...
                `${input.username}@${input.address}:${from_remote_dir}`, // FIXME ssh key file
                to_local_dir
            ]
        },
        {
