...
export async function rpc_ssh_install_docker<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_ssh_install_docker");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cloudman/ssh-install-docker`,
                data: input
            },
            {
