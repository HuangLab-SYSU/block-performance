...
export async function rpc_ssh_apt_install<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_ssh_apt_install");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cloudman/ssh-apt-install`,
                data: input
            },
            {
