...
import { ssh_apt_install as target_fun } from "./export.js";
import { rpc_ssh_apt_install as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            noninteractive: await skip_or_input("noninteractive", () =>
                input_noninteractive(log.sub("noninteractive"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            app_list: await input_app_list(log.sub("app_list"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_noninteractive<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_noninteractive");
        const v = await prompts.input_boolean("noninteractive");
        return cb.ok(v);
    }

    async function input_app_list<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_app_list");
        const length = await prompts.input_number("(array.length) app_list");
        const list: string[] = [];
