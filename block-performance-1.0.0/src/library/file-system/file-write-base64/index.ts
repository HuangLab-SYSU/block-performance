...
import { file_write_base64 as target_fun } from "./export.js";
import { rpc_file_write_base64 as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { name: string; base64: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { name: string; base64: string } = {
