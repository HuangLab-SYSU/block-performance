...
import { file_write_json as target_fun } from "./export.js";
import { rpc_file_write_json as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { name: string; json_value: any; json_format?: boolean }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { name: string; json_value: any; json_format?: boolean } = {
            ...
            json_value: await input_json_value(log.sub("json_value"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            json_format: await skip_or_input("json_format", () =>
                input_json_format(log.sub("json_format"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_json_value<R>(plog: Logger, cb: { ok: (v: any) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_json_value");
        const json_text = await prompts.input_string("json_value", { allow_empty: false });
        try {
    ...
    async function input_json_format<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_json_format");
        const v = await prompts.input_boolean("json_format");
        return cb.ok(v);
    }
