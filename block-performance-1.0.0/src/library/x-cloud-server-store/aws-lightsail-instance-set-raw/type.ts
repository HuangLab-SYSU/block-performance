...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        log.println("v.aws_lightsail_instance.id must be string");
        if (typeof v.aws_lightsail_instance.id !== "string") {
            throw new Error("v.aws_lightsail_instance.id is not string");
        }
