...
export async function rpc_admin_set<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_admin_set");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-cloud-server-store/admin-set`,
                data: input
            },
            {
