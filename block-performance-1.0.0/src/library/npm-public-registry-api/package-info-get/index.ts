...
import { package_info_get as target_fun } from "./export.js";
import { rpc_package_info_get as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { name: string; version?: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { name: string; version?: string } = {
            ...
            version: await skip_or_input("version", () =>
                input_version(log.sub("version"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
