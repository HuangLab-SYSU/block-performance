...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "includeDefaultKeyPair") return;
        ...
        if (v.includeDefaultKeyPair !== undefined) {
            log.println("v.includeDefaultKeyPair must be boolean");
            if (typeof v.includeDefaultKeyPair !== "boolean") {
                throw new Error("v.includeDefaultKeyPair is not boolean");
            }
        }
...
export function _copy_output_ok(v: any): OutputOk {
    if (typeof v === "object" && v !== null) {
        const obj = {
            keyPairs: copy_keyPairs(v.keyPairs),
            nextPageToken: copy_nextPageToken(v.nextPageToken)
        };
        return obj;
    } else {
        return { keyPairs: [], nextPageToken: "" };
    }

    function copy_keyPairs(
        v: any
    ): {
        arn: string;
        createdAt: string;
