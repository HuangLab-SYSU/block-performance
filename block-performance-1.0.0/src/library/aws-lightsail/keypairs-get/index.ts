...
import { keypairs_get as target_fun } from "./export.js";
import { rpc_keypairs_get as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; includeDefaultKeyPair?: boolean; pageToken?: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; includeDefaultKeyPair?: boolean; pageToken?: string } = {
            ...
            includeDefaultKeyPair: await skip_or_input("includeDefaultKeyPair", () =>
                input_includeDefaultKeyPair(log.sub("includeDefaultKeyPair"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            pageToken: await skip_or_input("pageToken", () =>
                input_pageToken(log.sub("pageToken"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_includeDefaultKeyPair<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_includeDefaultKeyPair");
        const v = await prompts.input_boolean("includeDefaultKeyPair");
        return cb.ok(v);
    }

    async function input_pageToken<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_pageToken");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("pageToken", { allow_empty: true });
        return cb.ok(v);
    }
