...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        log.println("v.instanceNames must be array");
        if (!Array.isArray(v.instanceNames)) {
            throw new Error("v.instanceNames is not array");
        }

        v.instanceNames.forEach((item: any, i: number) => {
            log.println("check v.instanceNames[i]");
        ...
        if (v.ipAddressType !== undefined) {
            log.println("v.ipAddressType must be string");
            if (typeof v.ipAddressType !== "string") {
                throw new Error("v.ipAddressType is not string");
            }

            if (new Set(["dualstack", "ipv4"]).has(v.ipAddressType) === false) {
                throw new Error("v.ipAddressType is not a valid string enum value");
            }
        }
    } catch (err) {
        log.error(err);
