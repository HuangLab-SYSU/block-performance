...
import { instances_create as target_fun } from "./export.js";
import { rpc_instances_create as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            instanceNames: await input_instanceNames(log.sub("instanceNames"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            availabilityZone: await input_availabilityZone(log.sub("availabilityZone"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            ...
            keyPairName: await skip_or_input("keyPairName", () =>
                input_keyPairName(log.sub("keyPairName"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            ipAddressType: await skip_or_input("ipAddressType", () =>
                input_ipAddressType(log.sub("ipAddressType"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_instanceNames<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_instanceNames");
        const length = await prompts.input_number("(array.length) instanceNames");
        const list: string[] = [];
    ...
    async function input_keyPairName<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_keyPairName");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("keyPairName", { allow_empty: true });
        return cb.ok(v);
    }
    ...
    async function input_ipAddressType<R>(plog: Logger, cb: { ok: (v: "dualstack" | "ipv4") => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_ipAddressType");
        // FIXME implement all string constrains here
        const v: "dualstack" | "ipv4" = await prompts.input_string_enum("ipAddressType", ["dualstack", "ipv4"]);
        return cb.ok(v);
    }
