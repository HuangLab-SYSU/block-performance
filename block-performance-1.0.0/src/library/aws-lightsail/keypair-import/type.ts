...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "publicKeyBase64") return;
        ...
        log.println("v.publicKeyBase64 must be string");
        if (typeof v.publicKeyBase64 !== "string") {
            throw new Error("v.publicKeyBase64 is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
