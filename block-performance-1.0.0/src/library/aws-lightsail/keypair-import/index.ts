...
import { keypair_import as target_fun } from "./export.js";
import { rpc_keypair_import as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; keyPairName: string; publicKeyBase64: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; keyPairName: string; publicKeyBase64: string } = {
            ...
            publicKeyBase64: await input_publicKeyBase64(log.sub("publicKeyBase64"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_publicKeyBase64<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_publicKeyBase64");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("publicKeyBase64", { allow_empty: true });
        return cb.ok(v);
    }
