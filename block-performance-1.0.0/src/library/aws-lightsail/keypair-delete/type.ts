...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "expectedFingerprint") return;
        ...
        if (v.expectedFingerprint !== undefined) {
            log.println("v.expectedFingerprint must be string");
            if (typeof v.expectedFingerprint !== "string") {
                throw new Error("v.expectedFingerprint is not string");
            }
        }
    } catch (err) {
        log.error(err);
