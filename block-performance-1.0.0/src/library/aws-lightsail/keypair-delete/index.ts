...
import { keypair_delete as target_fun } from "./export.js";
import { rpc_keypair_delete as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; keyPairName: string; expectedFingerprint?: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; keyPairName: string; expectedFingerprint?: string } = {
            ...
            keyPairName: await input_keyPairName(log.sub("keyPairName"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            expectedFingerprint: await skip_or_input("expectedFingerprint", () =>
                input_expectedFingerprint(log.sub("expectedFingerprint"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_expectedFingerprint<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_expectedFingerprint");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("expectedFingerprint", { allow_empty: true });
        return cb.ok(v);
    }
