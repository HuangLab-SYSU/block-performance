...
import { regions_get as target_fun } from "./export.js";
import { rpc_regions_get as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            includeAvailabilityZones: await skip_or_input("includeAvailabilityZones", () =>
                input_includeAvailabilityZones(log.sub("includeAvailabilityZones"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            includeRelationalDatabaseAvailabilityZones: await skip_or_input("includeRelationalDatabaseAvailabilityZones", () =>
                input_includeRelationalDatabaseAvailabilityZones(log.sub("includeRelationalDatabaseAvailabilityZones"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_includeAvailabilityZones<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_includeAvailabilityZones");
        const v = await prompts.input_boolean("includeAvailabilityZones");
        return cb.ok(v);
    }

    async function input_includeRelationalDatabaseAvailabilityZones<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_includeRelationalDatabaseAvailabilityZones");
        const v = await prompts.input_boolean("includeRelationalDatabaseAvailabilityZones");
        return cb.ok(v);
    }
