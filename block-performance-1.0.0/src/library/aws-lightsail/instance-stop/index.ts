...
import { instance_stop as target_fun } from "./export.js";
import { rpc_instance_stop as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; instanceName: string; force?: boolean }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; instanceName: string; force?: boolean } = {
            ...
            force: await skip_or_input("force", () =>
                input_force(log.sub("force"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_force<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_force");
        const v = await prompts.input_boolean("force");
        return cb.ok(v);
    }
