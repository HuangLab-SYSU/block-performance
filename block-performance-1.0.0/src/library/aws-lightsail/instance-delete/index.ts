...
import { instance_delete as target_fun } from "./export.js";
import { rpc_instance_delete as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; instanceName: string; forceDeleteAddOns?: boolean }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string; instanceName: string; forceDeleteAddOns?: boolean } = {
            ...
            forceDeleteAddOns: await skip_or_input("forceDeleteAddOns", () =>
                input_forceDeleteAddOns(log.sub("forceDeleteAddOns"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_forceDeleteAddOns<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_forceDeleteAddOns");
        const v = await prompts.input_boolean("forceDeleteAddOns");
        return cb.ok(v);
    }
