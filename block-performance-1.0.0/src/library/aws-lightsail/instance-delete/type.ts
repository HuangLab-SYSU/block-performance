...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "forceDeleteAddOns") return;
        ...
        if (v.forceDeleteAddOns !== undefined) {
            log.println("v.forceDeleteAddOns must be boolean");
            if (typeof v.forceDeleteAddOns !== "boolean") {
                throw new Error("v.forceDeleteAddOns is not boolean");
            }
        }
    } catch (err) {
        log.error(err);
