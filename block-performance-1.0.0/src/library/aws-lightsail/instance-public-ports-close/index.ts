...
import { instance_public_ports_close as target_fun } from "./export.js";
import { rpc_instance_public_ports_close as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            portInfo: await input_portInfo(log.sub("portInfo"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_portInfo<R>(
        plog: Logger,
        cb: {
            ok: (v: {
                protocol: "all" | "tcp" | "udp" | "icmp";
        ...
        const log = plog.sub("input_portInfo");
        try {
            var v: {
                protocol: "all" | "tcp" | "udp" | "icmp";
                fromPort: number;
