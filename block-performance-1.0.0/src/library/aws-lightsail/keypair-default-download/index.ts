...
import { keypair_default_download as target_fun } from "./export.js";
import { rpc_keypair_default_download as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region?: string } = {
