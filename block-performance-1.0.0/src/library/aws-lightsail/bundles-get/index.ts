...
import { bundles_get as target_fun } from "./export.js";
import { rpc_bundles_get as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credentials: { accessKeyId: string; secretAccessKey: string }; region: string; includeInactive?: boolean; pageToken?: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credentials: { accessKeyId: string; secretAccessKey: string }; region: string; includeInactive?: boolean; pageToken?: string } = {
            ...
            includeInactive: await skip_or_input("includeInactive", () =>
                input_includeInactive(log.sub("includeInactive"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_includeInactive<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_includeInactive");
        const v = await prompts.input_boolean("includeInactive");
        return cb.ok(v);
    }
