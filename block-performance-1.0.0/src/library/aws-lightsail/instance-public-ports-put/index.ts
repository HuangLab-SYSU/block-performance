...
import { instance_public_ports_put as target_fun } from "./export.js";
import { rpc_instance_public_ports_put as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            portInfos: await input_portInfos(log.sub("portInfos"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_portInfos<R>(
        plog: Logger,
        cb: {
            ok: (
                v: {
        ...
        const log = plog.sub("input_portInfos");
        const length = await prompts.input_number("(array.length) portInfos");
        const list: {
            protocol: "all" | "tcp" | "udp" | "icmp";
            fromPort: number;
            toPort: number;
