...
import { get_text as target_fun } from "./export.js";
import { rpc_get_text as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { url: string; url_params?: { [key: string]: string }; headers?: { [key: string]: string }; proxy_url: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { url: string; url_params?: { [key: string]: string }; headers?: { [key: string]: string }; proxy_url: string } = {
