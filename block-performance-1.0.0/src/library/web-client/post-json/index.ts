...
import { post_json as target_fun } from "./export.js";
import { rpc_post_json as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { url: string; url_params?: { [key: string]: string }; headers?: { [key: string]: string }; proxy_url: string; data: any }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { url: string; url_params?: { [key: string]: string }; headers?: { [key: string]: string }; proxy_url: string; data: any } = {
            url: await input_url(log.sub("url"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            url_params: await skip_or_input("url_params", () =>
                input_url_params(log.sub("url_params"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            headers: await skip_or_input("headers", () =>
                input_headers(log.sub("headers"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            proxy_url: await input_proxy_url(log.sub("proxy_url"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            data: await input_data(log.sub("data"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_url<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_url");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("url", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_url_params<R>(plog: Logger, cb: { ok: (v: { [key: string]: string }) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_url_params");
        try {
    ...
    async function input_headers<R>(plog: Logger, cb: { ok: (v: { [key: string]: string }) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_headers");
        try {
            var v: { [key: string]: string } = {};
        } catch (err) {
            log.error(err);
    ...
    async function input_proxy_url<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_proxy_url");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("proxy_url", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_data<R>(plog: Logger, cb: { ok: (v: any) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_data");
        const json_text = await prompts.input_string("data", { allow_empty: false });
        try {
            const v = JSON.parse(json_text);
            return cb.ok(v);
        } catch (err) {
            log.error(err);
            return cb.fail(err);
