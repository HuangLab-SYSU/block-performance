...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        if (v.filename !== undefined) {
            log.println("v.filename must be string");
            if (typeof v.filename !== "string") {
                throw new Error("v.filename is not string");
            }
        }
    } catch (err) {
        log.error(err);
...
export function _copy_output_unexpected_response(v: any): OutputUnexpectedResponse {
    if (typeof v === "object" && v !== null) {
        const obj = {
            ...
            temp_file_name: copy_temp_file_name(v.temp_file_name)
        };
        return obj;
    } else {
        return { status_code: 0, status_message: "", headers: [], temp_file_name: "" };
    }
    ...
    function copy_temp_file_name(v: any): string {
        return typeof v === "string" ? v : "";
    }
}
