...
import { Input, Output, check_input, OutputUnexpectedResponse, OutputOk } from "./type.js";
import { get_file_temp as target_fun } from "./export.js";
import { rpc_get_file_temp as rpc_target_fun } from "./rpc/client.js";
    ...
    try {
        ...
        if (server) {
            ...
            await target_fun(log, input, {
                unexpected_response: (output: OutputUnexpectedResponse) => {
                    console.log(JSON.stringify(output, null, 4));
                    console.log("😄 unexpected_response");
                },
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { url: string; headers?: { name: string; value: string }[]; filename?: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { url: string; headers?: { name: string; value: string }[]; filename?: string } = {
            ...
            filename: await skip_or_input("filename", () =>
                input_filename(log.sub("filename"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_headers<R>(plog: Logger, cb: { ok: (v: { name: string; value: string }[]) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_headers");
        const length = await prompts.input_number("(array.length) headers");
        ...
        async function input_item<R>(plog: Logger, cb: { ok: (v: { name: string; value: string }) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_item");
            try {
                var v: { name: string; value: string } = {
    ...
    async function input_filename<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_filename");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("filename", { allow_empty: true });
        return cb.ok(v);
    }
