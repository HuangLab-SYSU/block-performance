...
import { Input, Callback, OutputUnexpectedResponse, OutputOk } from "../type.js";
import { RpcResult } from "./type.js";

export async function rpc_get_file_temp<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_get_file_temp");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/web-client/get-file-temp`,
                data: input
            },
            {
                ok: (rpc_result: RpcResult) => {
                    ...
                    if (rpc_result.unexpected_response) {
                        log.println("unexpected_response");
                        const output: OutputUnexpectedResponse = rpc_result.unexpected_response;
