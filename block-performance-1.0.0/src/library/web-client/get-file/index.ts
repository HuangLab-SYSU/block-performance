...
import { get_file as target_fun } from "./export.js";
import { rpc_get_file as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { url: string; headers?: { name: string; value: string }[]; save_to_file: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { url: string; headers?: { name: string; value: string }[]; save_to_file: string } = {
            ...
            save_to_file: await input_save_to_file(log.sub("save_to_file"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_save_to_file<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_save_to_file");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("save_to_file", { allow_empty: true });
        return cb.ok(v);
    }
