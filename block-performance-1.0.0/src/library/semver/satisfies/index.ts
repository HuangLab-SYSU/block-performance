...
import { satisfies as target_fun } from "./export.js";
import { rpc_satisfies as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { version: string; range: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { version: string; range: string } = {
            ...
            range: await input_range(log.sub("range"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_range<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_range");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("range", { allow_empty: true });
        return cb.ok(v);
    }
}
