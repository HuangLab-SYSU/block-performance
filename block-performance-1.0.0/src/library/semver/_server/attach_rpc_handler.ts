...
import { handle_rpc_satisfies } from "../satisfies/rpc/handle.js";

export function attach_rpc_handler(plog: Logger, opt: { router: express.Router }) {
    const log = plog.sub("server.attach_rpc_handler");
    const { router } = opt;

    router.post("/library/semver/parse", (req, res) => {
        const input = req.body;
        const req_log = log.sub("post.library.semver.parse");
    ...
    router.post("/library/semver/satisfies", (req, res) => {
        const input = req.body;
        const req_log = log.sub("post.library.semver.satisfies");
        handle_rpc_satisfies(req_log, input, {
            invalid_input: (err) => {
                // bad request
                req_log.error(err);
                res.status(400);
