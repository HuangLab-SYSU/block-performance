    ...
    try {
        ...
        if (v.sort !== undefined) {
            ...
            v.sort.field_list.forEach((item: any, i: number) => {
                ...
                if (item.aws_lightsail !== undefined) {
                    log.println("item.aws_lightsail must be object");
                    if (typeof item.aws_lightsail !== "object" || item.aws_lightsail === null) {
                        throw new Error("item.aws_lightsail is not object");
                    }

                    Object.keys(item.aws_lightsail).forEach((field) => {
                        if (field === "access_key_id") return;
                        if (field === "secret_access_key") return;
                        throw new Error("item.aws_lightsail contains unknown field: " + field);
                    });

                    if (item.aws_lightsail.access_key_id !== undefined) {
                        log.println("item.aws_lightsail.access_key_id must be object");
                        if (typeof item.aws_lightsail.access_key_id !== "object" || item.aws_lightsail.access_key_id === null) {
                            throw new Error("item.aws_lightsail.access_key_id is not object");
                        }

                        Object.keys(item.aws_lightsail.access_key_id).forEach((field) => {
                            if (field === "order") return;
                            throw new Error("item.aws_lightsail.access_key_id contains unknown field: " + field);
                        });

                        log.println("item.aws_lightsail.access_key_id.order must be string");
                        if (typeof item.aws_lightsail.access_key_id.order !== "string") {
                            throw new Error("item.aws_lightsail.access_key_id.order is not string");
                        }

                        if (new Set(["ascending", "descending"]).has(item.aws_lightsail.access_key_id.order) === false) {
                            throw new Error("item.aws_lightsail.access_key_id.order is not a valid string enum value");
                        }
                    }

                    if (item.aws_lightsail.secret_access_key !== undefined) {
                        log.println("item.aws_lightsail.secret_access_key must be object");
                        if (typeof item.aws_lightsail.secret_access_key !== "object" || item.aws_lightsail.secret_access_key === null) {
                            throw new Error("item.aws_lightsail.secret_access_key is not object");
                        }

                        Object.keys(item.aws_lightsail.secret_access_key).forEach((field) => {
                            if (field === "order") return;
                            throw new Error("item.aws_lightsail.secret_access_key contains unknown field: " + field);
                        });

                        log.println("item.aws_lightsail.secret_access_key.order must be string");
                        if (typeof item.aws_lightsail.secret_access_key.order !== "string") {
                            throw new Error("item.aws_lightsail.secret_access_key.order is not string");
                        }

                        if (new Set(["ascending", "descending"]).has(item.aws_lightsail.secret_access_key.order) === false) {
                            throw new Error("item.aws_lightsail.secret_access_key.order is not a valid string enum value");
                        }
                    }
                }
            });
