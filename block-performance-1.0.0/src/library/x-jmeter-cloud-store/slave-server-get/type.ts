...
export function _copy_output_none(v: any): OutputNone {
    if (typeof v === "object" && v !== null) {
        const obj = {
            slave_server: copy_slave_server(v.slave_server)
        };
        return obj;
    } else {
        return { slave_server: { id: "", create_time: "", update_time: "", cloud_server: { aws_lightsail_instance_id: "" } } };
    }

    function copy_slave_server(v: any): {
        id: string;
        fake?: boolean;
        create_time: string;
        update_time: string;
