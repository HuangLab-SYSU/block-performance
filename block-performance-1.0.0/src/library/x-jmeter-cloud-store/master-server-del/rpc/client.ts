...
export async function rpc_master_server_del<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_master_server_del");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-store/master-server-del`,
                data: input
            },
            {
