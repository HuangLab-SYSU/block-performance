...
export interface OutputEmpty {
    list: { id: string; fake?: boolean; create_time: string; update_time: string; cloud_server: { aws_lightsail_instance_id: string } }[];
    total_count: number;
}
    ...
    try {
        ...
        if (v.sort !== undefined) {
            ...
            v.sort.field_list.forEach((item: any, i: number) => {
                ...
                if (item.cloud_server !== undefined) {
                    log.println("item.cloud_server must be object");
                    if (typeof item.cloud_server !== "object" || item.cloud_server === null) {
                        throw new Error("item.cloud_server is not object");
                    }

                    Object.keys(item.cloud_server).forEach((field) => {
                        if (field === "aws_lightsail_instance_id") return;
                        throw new Error("item.cloud_server contains unknown field: " + field);
                    });

                    if (item.cloud_server.aws_lightsail_instance_id !== undefined) {
                        log.println("item.cloud_server.aws_lightsail_instance_id must be object");
                        if (typeof item.cloud_server.aws_lightsail_instance_id !== "object" || item.cloud_server.aws_lightsail_instance_id === null) {
                            throw new Error("item.cloud_server.aws_lightsail_instance_id is not object");
                        }

                        Object.keys(item.cloud_server.aws_lightsail_instance_id).forEach((field) => {
                            if (field === "order") return;
                            throw new Error("item.cloud_server.aws_lightsail_instance_id contains unknown field: " + field);
                        });

                        log.println("item.cloud_server.aws_lightsail_instance_id.order must be string");
                        if (typeof item.cloud_server.aws_lightsail_instance_id.order !== "string") {
                            throw new Error("item.cloud_server.aws_lightsail_instance_id.order is not string");
                        }

                        if (new Set(["ascending", "descending"]).has(item.cloud_server.aws_lightsail_instance_id.order) === false) {
                            throw new Error("item.cloud_server.aws_lightsail_instance_id.order is not a valid string enum value");
                        }
                    }
                }
            });
