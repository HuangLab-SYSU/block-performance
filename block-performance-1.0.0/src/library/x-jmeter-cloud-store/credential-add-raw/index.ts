...
import { credential_add_raw as target_fun } from "./export.js";
import { rpc_credential_add_raw as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    async function input_credential<R>(
        plog: Logger,
        cb: {
            ok: (v: { fake?: boolean; create_time: string; update_time: string; aws_lightsail: { access_key_id: string; secret_access_key: string } }) => R;
            fail: (err: Error) => R;
        }
    ): Promise<R> {
        const log = plog.sub("input_credential");
        try {
            var v: { fake?: boolean; create_time: string; update_time: string; aws_lightsail: { access_key_id: string; secret_access_key: string } } = {
