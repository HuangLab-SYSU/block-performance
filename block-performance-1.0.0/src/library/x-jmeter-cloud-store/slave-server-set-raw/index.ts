...
import { slave_server_set_raw as target_fun } from "./export.js";
import { rpc_slave_server_set_raw as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: { slave_server: { id: string; fake?: boolean; create_time: string; update_time: string; cloud_server: { aws_lightsail_instance_id: string } } } =
            {
                slave_server: await input_slave_server(log.sub("slave_server"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_slave_server<R>(
        plog: Logger,
        cb: {
            ok: (v: { id: string; fake?: boolean; create_time: string; update_time: string; cloud_server: { aws_lightsail_instance_id: string } }) => R;
            fail: (err: Error) => R;
        }
    ): Promise<R> {
        const log = plog.sub("input_slave_server");
        try {
            var v: { id: string; fake?: boolean; create_time: string; update_time: string; cloud_server: { aws_lightsail_instance_id: string } } = {
                ...
                cloud_server: await input_cloud_server(log.sub("cloud_server"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
        ...
        async function input_cloud_server<R>(plog: Logger, cb: { ok: (v: { aws_lightsail_instance_id: string }) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_cloud_server");
            try {
                var v: { aws_lightsail_instance_id: string } = {
                    aws_lightsail_instance_id: await input_aws_lightsail_instance_id(log.sub("aws_lightsail_instance_id"), {
                        ok: (v) => v,
                        fail: (err) => {
                            throw err;
                        }
            ...
            async function input_aws_lightsail_instance_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
                const log = plog.sub("input_aws_lightsail_instance_id");
                // FIXME implement all string constrains here
                const v = await prompts.input_string("aws_lightsail_instance_id", { allow_empty: true });
                return cb.ok(v);
            }
        }
