...
function make_root_sort_fun(
    ...
    [
        ...
        sort_option.update_time
            ...
            : undefined,
        sort_option.aws_lightsail
            ? make_field_sort_fun_aws_lightsail(log, {
                  sort_option: sort_option.aws_lightsail,
                  get_value: (data) => {
                      const v = get_value(data);
                      return v ? v.aws_lightsail : undefined;
                  }
              })
            : undefined
    ].forEach((item) => {
    ...
    function make_field_sort_fun_aws_lightsail(
        plog: Logger,
        opt: {
            sort_option: { access_key_id?: { order: "ascending" | "descending" }; secret_access_key?: { order: "ascending" | "descending" } };
            get_value: (item: Data) => { access_key_id: string; secret_access_key: string } | undefined; // undefined for optional field
        }
    ): SortFun | undefined {
        const log = plog.sub("make_field_sort_fun_aws_lightsail");
        const { sort_option, get_value } = opt;
        ...
        // check each field and make sort function for it if needed
        [
            sort_option.access_key_id
                ? make_field_sort_fun_access_key_id(log, {
                      sort_option: sort_option.access_key_id,
                      get_value: (data) => {
                          const v = get_value(data);
                          return v ? v.access_key_id : undefined;
                      }
                  })
                : undefined,
            sort_option.secret_access_key
                ? make_field_sort_fun_secret_access_key(log, {
                      sort_option: sort_option.secret_access_key,
                      get_value: (data) => {
                          const v = get_value(data);
                          return v ? v.secret_access_key : undefined;
                      }
                  })
                : undefined
        ].forEach((item) => {
        ...
        function make_field_sort_fun_access_key_id(
            plog: Logger,
            opt: {
                sort_option: { order: "ascending" | "descending" };
                get_value: (item: Data) => string | undefined; // undefined for optional field
            ...
            const log = plog.sub("make_field_sort_fun_access_key_id");
            const { sort_option, get_value } = opt;
        ...
        function make_field_sort_fun_secret_access_key(
            plog: Logger,
            opt: {
                sort_option: { order: "ascending" | "descending" };
                get_value: (item: Data) => string | undefined; // undefined for optional field
            ...
            const log = plog.sub("make_field_sort_fun_secret_access_key");
            const { sort_option, get_value } = opt;
