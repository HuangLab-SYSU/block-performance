...
import { credential_set as target_fun } from "./export.js";
import { rpc_credential_set as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { credential: { id: string; fake?: boolean; aws_lightsail: { access_key_id: string; secret_access_key: string } } }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credential: { id: string; fake?: boolean; aws_lightsail: { access_key_id: string; secret_access_key: string } } } = {
    ...
    async function input_credential<R>(
        plog: Logger,
        cb: { ok: (v: { id: string; fake?: boolean; aws_lightsail: { access_key_id: string; secret_access_key: string } }) => R; fail: (err: Error) => R }
    ): Promise<R> {
        const log = plog.sub("input_credential");
        try {
            var v: { id: string; fake?: boolean; aws_lightsail: { access_key_id: string; secret_access_key: string } } = {
