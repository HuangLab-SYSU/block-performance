...
import { master_server_ls_raw as target_fun } from "./export.js";
import { rpc_master_server_ls_raw as rpc_target_fun } from "./rpc/client.js";
            ...
            async function input_item<R>(
                ...
                try {
                    var v: {
                        ...
                        cloud_server: await skip_or_input("cloud_server", () =>
                            input_cloud_server(log.sub("cloud_server"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                ...
                async function input_cloud_server<R>(
                    plog: Logger,
                    cb: { ok: (v: { aws_lightsail_instance_id?: { order: "ascending" | "descending" } }) => R; fail: (err: Error) => R }
                ): Promise<R> {
                    const log = plog.sub("input_cloud_server");
                    try {
                        var v: { aws_lightsail_instance_id?: { order: "ascending" | "descending" } } = {
                            aws_lightsail_instance_id: await skip_or_input("aws_lightsail_instance_id", () =>
                                input_aws_lightsail_instance_id(log.sub("aws_lightsail_instance_id"), {
                                    ok: (v) => v,
                                    fail: (err) => {
                                        throw err;
                                    }
                    ...
                    async function input_aws_lightsail_instance_id<R>(
                        plog: Logger,
                        cb: { ok: (v: { order: "ascending" | "descending" }) => R; fail: (err: Error) => R }
                    ): Promise<R> {
