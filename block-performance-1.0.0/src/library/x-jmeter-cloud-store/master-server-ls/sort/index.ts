...
interface SortRequest {
    field_list: {
        ...
        cloud_server?: { aws_lightsail_instance_id?: { order: "ascending" | "descending" } };
    }[];
}
...
function make_root_sort_fun(
    ...
    opt: {
        ...
        get_value: (
            item: Data
        ) => { id: string; fake?: boolean; create_time: string; update_time: string; cloud_server: { aws_lightsail_instance_id: string } } | undefined; // undefined for optional field
    }
): SortFun | undefined {
    ...
    [
        ...
        sort_option.update_time
            ...
            : undefined,
        sort_option.cloud_server
            ? make_field_sort_fun_cloud_server(log, {
                  sort_option: sort_option.cloud_server,
                  get_value: (data) => {
                      const v = get_value(data);
                      return v ? v.cloud_server : undefined;
                  }
              })
            : undefined
    ].forEach((item) => {
    ...
    function make_field_sort_fun_cloud_server(
        plog: Logger,
        opt: {
            sort_option: { aws_lightsail_instance_id?: { order: "ascending" | "descending" } };
            get_value: (item: Data) => { aws_lightsail_instance_id: string } | undefined; // undefined for optional field
        }
    ): SortFun | undefined {
        const log = plog.sub("make_field_sort_fun_cloud_server");
        const { sort_option, get_value } = opt;
        ...
        // check each field and make sort function for it if needed
        [
            sort_option.aws_lightsail_instance_id
                ? make_field_sort_fun_aws_lightsail_instance_id(log, {
                      sort_option: sort_option.aws_lightsail_instance_id,
                      get_value: (data) => {
                          const v = get_value(data);
                          return v ? v.aws_lightsail_instance_id : undefined;
                      }
                  })
                : undefined
        ].forEach((item) => {
        ...
        function make_field_sort_fun_aws_lightsail_instance_id(
            plog: Logger,
            opt: {
                sort_option: { order: "ascending" | "descending" };
                get_value: (item: Data) => string | undefined; // undefined for optional field
            ...
            const log = plog.sub("make_field_sort_fun_aws_lightsail_instance_id");
            const { sort_option, get_value } = opt;
