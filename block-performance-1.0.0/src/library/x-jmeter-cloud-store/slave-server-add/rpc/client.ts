...
export async function rpc_slave_server_add<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_slave_server_add");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-store/slave-server-add`,
                data: input
            },
            {
