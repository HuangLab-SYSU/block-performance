...
import { target_server_add as target_fun } from "./export.js";
import { rpc_target_server_add as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { target_server: { fake?: boolean; cloud_server: { aws_lightsail_instance_id: string } } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { target_server: { fake?: boolean; cloud_server: { aws_lightsail_instance_id: string } } } = {
