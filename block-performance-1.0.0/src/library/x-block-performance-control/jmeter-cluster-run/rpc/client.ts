...
export async function rpc_jmeter_cluster_run<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_jmeter_cluster_run");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-block-performance-control/jmeter-cluster-run`,
                data: input
            },
            {
