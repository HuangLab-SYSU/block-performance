...
import { jmeter_cluster_run as target_fun } from "./export.js";
import { rpc_jmeter_cluster_run as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { local_test_plan_file_path: string; download_jtl_file_to_path: string; download_html_report_directory_to_path: string }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { local_test_plan_file_path: string; download_jtl_file_to_path: string; download_html_report_directory_to_path: string } = {
