...
import { jmeter_cluster_create as target_fun } from "./export.js";
import { rpc_jmeter_cluster_create as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { slave_server_count: number; credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { slave_server_count: number; credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } } = {
            slave_server_count: await input_slave_server_count(log.sub("slave_server_count"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_slave_server_count<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_slave_server_count");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("slave_server_count");
        return cb.ok(v);
    }
