...
export async function rpc_jmeter_cluster_remove<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_jmeter_cluster_remove");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-block-performance-control/jmeter-cluster-remove`,
                data: input
            },
            {
