...
import { jmeter_cluster_remove as target_fun } from "./export.js";
import { rpc_jmeter_cluster_remove as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } } = {
