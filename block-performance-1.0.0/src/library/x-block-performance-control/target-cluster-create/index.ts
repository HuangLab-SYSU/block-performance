...
import { target_cluster_create as target_fun } from "./export.js";
import { rpc_target_cluster_create as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { target_server_count: number; credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { target_server_count: number; credential: { aws_lightsail: { access_key_id: string; secret_access_key: string } } } = {
            target_server_count: await input_target_server_count(log.sub("target_server_count"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            credential: await input_credential(log.sub("credential"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_target_server_count<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_target_server_count");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("target_server_count");
        return cb.ok(v);
    }

    async function input_credential<R>(
        plog: Logger,
        cb: { ok: (v: { aws_lightsail: { access_key_id: string; secret_access_key: string } }) => R; fail: (err: Error) => R }
    ): Promise<R> {
        const log = plog.sub("input_credential");
        try {
            var v: { aws_lightsail: { access_key_id: string; secret_access_key: string } } = {
                aws_lightsail: await input_aws_lightsail(log.sub("aws_lightsail"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
        ...
        async function input_aws_lightsail<R>(
            plog: Logger,
            cb: { ok: (v: { access_key_id: string; secret_access_key: string }) => R; fail: (err: Error) => R }
        ): Promise<R> {
            const log = plog.sub("input_aws_lightsail");
            try {
                var v: { access_key_id: string; secret_access_key: string } = {
                    access_key_id: await input_access_key_id(log.sub("access_key_id"), {
                        ok: (v) => v,
                        fail: (err) => {
                            throw err;
                        }
                    }),
                    secret_access_key: await input_secret_access_key(log.sub("secret_access_key"), {
                        ok: (v) => v,
                        fail: (err) => {
                            throw err;
                        }
            ...
            async function input_access_key_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
                const log = plog.sub("input_access_key_id");
                // FIXME implement all string constrains here
                const v = await prompts.input_string("access_key_id", { allow_empty: true });
                return cb.ok(v);
            }

            async function input_secret_access_key<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
                const log = plog.sub("input_secret_access_key");
                // FIXME implement all string constrains here
                const v = await prompts.input_string("secret_access_key", { allow_empty: true });
                return cb.ok(v);
            }
        }
    }
