...
import { search_params_merge as target_fun } from "./export.js";
import { rpc_search_params_merge as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { url: string; search_params?: { [key: string]: string } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { url: string; search_params?: { [key: string]: string } } = {
