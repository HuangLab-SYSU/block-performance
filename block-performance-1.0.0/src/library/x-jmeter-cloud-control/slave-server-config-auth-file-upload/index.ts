...
import { slave_server_config_auth_file_upload as target_fun } from "./export.js";
import { rpc_slave_server_config_auth_file_upload as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { slave_server_id: string; local_auth_file_path: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { slave_server_id: string; local_auth_file_path: string } = {
            slave_server_id: await input_slave_server_id(log.sub("slave_server_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_slave_server_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_slave_server_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("slave_server_id", { allow_empty: true });
        return cb.ok(v);
    }
