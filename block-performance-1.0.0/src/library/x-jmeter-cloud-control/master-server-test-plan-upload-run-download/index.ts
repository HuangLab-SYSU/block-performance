...
import { master_server_test_plan_upload_run_download as target_fun } from "./export.js";
import { rpc_master_server_test_plan_upload_run_download as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            local_test_plan_file_path: await input_local_test_plan_file_path(log.sub("local_test_plan_file_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            download_jtl_file_to_path: await input_download_jtl_file_to_path(log.sub("download_jtl_file_to_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            download_html_report_directory_to_path: await input_download_html_report_directory_to_path(log.sub("download_html_report_directory_to_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_local_test_plan_file_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_local_test_plan_file_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("local_test_plan_file_path", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_download_jtl_file_to_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_download_jtl_file_to_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("download_jtl_file_to_path", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_download_html_report_directory_to_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_download_html_report_directory_to_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("download_html_report_directory_to_path", { allow_empty: true });
        return cb.ok(v);
    }
}
