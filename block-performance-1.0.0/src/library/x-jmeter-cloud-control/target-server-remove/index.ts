...
import { target_server_remove as target_fun } from "./export.js";
import { rpc_target_server_remove as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { target_server_id: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { target_server_id: string } = {
            target_server_id: await input_target_server_id(log.sub("target_server_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_target_server_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_target_server_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("target_server_id", { allow_empty: true });
        return cb.ok(v);
    }
}
