...
export async function rpc_target_server_remove<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_target_server_remove");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/target-server-remove`,
                data: input
            },
            {
