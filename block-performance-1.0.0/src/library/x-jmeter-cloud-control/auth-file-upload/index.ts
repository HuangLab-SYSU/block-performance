...
import { auth_file_upload as target_fun } from "./export.js";
import { rpc_auth_file_upload as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            local_auth_file_path: await input_local_auth_file_path(log.sub("local_auth_file_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            remote_auth_file_path: await input_remote_auth_file_path(log.sub("remote_auth_file_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_local_auth_file_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_local_auth_file_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("local_auth_file_path", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_remote_auth_file_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_remote_auth_file_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("remote_auth_file_path", { allow_empty: true });
        return cb.ok(v);
    }
}
