...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "local_auth_file_path") return;
            if (field === "remote_auth_file_path") return;
        ...
        log.println("v.local_auth_file_path must be string");
        if (typeof v.local_auth_file_path !== "string") {
            throw new Error("v.local_auth_file_path is not string");
        }

        log.println("v.remote_auth_file_path must be string");
        if (typeof v.remote_auth_file_path !== "string") {
            throw new Error("v.remote_auth_file_path is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
