...
export async function rpc_master_server_test_plan_output_download<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_master_server_test_plan_output_download");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/master-server-test-plan-output-download`,
                data: input
            },
            {
