...
export async function rpc_master_server_create_install_config<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_master_server_create_install_config");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/master-server-create-install-config`,
                data: input
            },
            {
