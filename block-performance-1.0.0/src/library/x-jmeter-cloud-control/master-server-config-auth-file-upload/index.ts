...
import { master_server_config_auth_file_upload as target_fun } from "./export.js";
import { rpc_master_server_config_auth_file_upload as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { master_server_id: string; local_auth_file_path: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { master_server_id: string; local_auth_file_path: string } = {
