...
export async function rpc_master_server_config_auth_file_upload<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_master_server_config_auth_file_upload");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/master-server-config-auth-file-upload`,
                data: input
            },
            {
