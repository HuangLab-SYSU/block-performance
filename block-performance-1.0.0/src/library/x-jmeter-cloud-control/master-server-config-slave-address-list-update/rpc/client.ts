...
export async function rpc_master_server_config_slave_address_list_update<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_master_server_config_slave_address_list_update");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/master-server-config-slave-address-list-update`,
                data: input
            },
            {
