...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    return await store.master_server_get(
        log,
        {
            id: input.master_server_id
        },
        {
            none: async () => {
                return cb.none();
            },
            ok: async ({ master_server }) => {
                await cloud_server.instance_remove(
                    log,
                    {
                        credentials,
                        id: master_server.cloud_server.aws_lightsail_instance_id
                    },
                    {
                        none: () => {
                            // ignore
