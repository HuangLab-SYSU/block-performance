...
import { master_server_test_plan_run as target_fun } from "./export.js";
import { rpc_master_server_test_plan_run as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { master_server_id: string; remote_test_plan_file_path: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { master_server_id: string; remote_test_plan_file_path: string } = {
            ...
            remote_test_plan_file_path: await input_remote_test_plan_file_path(log.sub("remote_test_plan_file_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_remote_test_plan_file_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_remote_test_plan_file_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("remote_test_plan_file_path", { allow_empty: true });
        return cb.ok(v);
    }
}
