...
import { slave_server_create_install_config_run as target_fun } from "./export.js";
import { rpc_slave_server_create_install_config_run as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { local_auth_file_path: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { local_auth_file_path: string } = {
