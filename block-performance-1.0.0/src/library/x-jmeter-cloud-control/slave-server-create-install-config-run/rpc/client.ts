...
export async function rpc_slave_server_create_install_config_run<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_slave_server_create_install_config_run");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-jmeter-cloud-control/slave-server-create-install-config-run`,
                data: input
            },
            {
