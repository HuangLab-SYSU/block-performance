...
export async function rpc_key_add_raw<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_key_add_raw");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-ssh-key-store/key-add-raw`,
                data: input
            },
            {
