...
import { key_ls_raw as target_fun } from "./export.js";
import { rpc_key_ls_raw as rpc_target_fun } from "./rpc/client.js";
            ...
            async function input_item<R>(
                ...
                try {
                    var v: {
                        ...
                        address_list: await skip_or_input("address_list", () =>
                            input_address_list(log.sub("address_list"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                        ...
                        tag_list: await skip_or_input("tag_list", () =>
                            input_tag_list(log.sub("tag_list"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                        ...
                        private_key: await skip_or_input("private_key", () =>
                            input_private_key(log.sub("private_key"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                        ...
                        disabled: await skip_or_input("disabled", () =>
                            input_disabled(log.sub("disabled"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                ...
                async function input_address_list<R>(
                    plog: Logger,
                    cb: { ok: (v: { index: number; item: { order: "ascending" | "descending" } }) => R; fail: (err: Error) => R }
                ): Promise<R> {
                    const log = plog.sub("input_address_list");
                    try {
                        var v: { index: number; item: { order: "ascending" | "descending" } } = {
                            index: await input_index(log.sub("index"), {
                                ok: (v) => v,
                                fail: (err) => {
                                    throw err;
                                }
                    ...
                    async function input_index<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
                        const log = plog.sub("input_index");
                        // FIXME implement all number constrains here
                        const v = await prompts.input_number("index");
                        return cb.ok(v);
                    }
                ...
                async function input_tag_list<R>(
                    plog: Logger,
                    cb: { ok: (v: { index: number; item: { order: "ascending" | "descending" } }) => R; fail: (err: Error) => R }
                ): Promise<R> {
                ...
                async function input_private_key<R>(
                    plog: Logger,
                    cb: { ok: (v: { order: "ascending" | "descending" }) => R; fail: (err: Error) => R }
                ): Promise<R> {
                ...
                async function input_disabled<R>(
                    plog: Logger,
                    cb: { ok: (v: { order: "ascending" | "descending" }) => R; fail: (err: Error) => R }
                ): Promise<R> {
