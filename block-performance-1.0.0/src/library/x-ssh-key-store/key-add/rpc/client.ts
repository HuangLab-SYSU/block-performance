...
export async function rpc_key_add<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_key_add");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-ssh-key-store/key-add`,
                data: input
            },
            {
