...
import { key_add as target_fun } from "./export.js";
import { rpc_key_add as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { key: { fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean } }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { key: { fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean } } = {
    ...
    async function input_key<R>(
        plog: Logger,
        cb: {
            ok: (v: { fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean }) => R;
            fail: (err: Error) => R;
        }
    ): Promise<R> {
        const log = plog.sub("input_key");
        try {
            var v: { fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean } = {
