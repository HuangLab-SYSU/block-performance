...
import { key_set as target_fun } from "./export.js";
import { rpc_key_set as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: { key: { id: string; fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean } } = {
    ...
    async function input_key<R>(
        plog: Logger,
        cb: {
            ok: (v: { id: string; fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean }) => R;
            fail: (err: Error) => R;
        }
    ): Promise<R> {
        const log = plog.sub("input_key");
        try {
            var v: { id: string; fake?: boolean; username: string; address_list: string[]; tag_list: string[]; private_key: string; disabled: boolean } = {
                ...
                address_list: await input_address_list(log.sub("address_list"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
                }),
                tag_list: await input_tag_list(log.sub("tag_list"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
                }),
                private_key: await input_private_key(log.sub("private_key"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
                }),
                disabled: await input_disabled(log.sub("disabled"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
        ...
        async function input_address_list<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_address_list");
            const length = await prompts.input_number("(array.length) address_list");
            const list: string[] = [];
        ...
        async function input_tag_list<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_tag_list");
            const length = await prompts.input_number("(array.length) tag_list");
            const list: string[] = [];
            for (let i = 0; i < length; ++i) {
                const err = await input_item(log, {
                    ok: (item) => {
        ...
        async function input_private_key<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_private_key");
            // FIXME implement all string constrains here
            const v = await prompts.input_string("private_key", { allow_empty: true });
            return cb.ok(v);
        }

        async function input_disabled<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
            const log = plog.sub("input_disabled");
            const v = await prompts.input_boolean("disabled");
            return cb.ok(v);
        }
